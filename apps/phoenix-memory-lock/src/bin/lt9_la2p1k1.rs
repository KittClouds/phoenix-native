//! LT9-LA2-P1K1: descriptive audit of fixed-priority phenotype assignment.
//!
//! Reconstructs the sealed P1J3 eight-shard roster, verifies its 67 same-
//! phenotype records against the immutable pair roster, and emits endpoint
//! assignment diagnostics plus one plurality/tie-abstain counterfactual.
//! It never updates learner authority or applies a compatibility guard.
...
fn main() -> Result<()> {
    let output_path = env::args()
        .nth(5)
        .unwrap_or_else(|| "D:\\phoenix-evals\\lt9-la2p1k1\\lt9-la2p1k1-receipt.json".to_owned());
    let receipt = main_run()?;
    let path = Path::new(&output_path);
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)?;
    }
    fs::write(path, serde_json::to_vec_pretty(&receipt)?)?;
    println!("P1K1 diagnostic receipt: {}", path.display());
    println!(
        "pairs={} episodes={} inversions={} invalid-actionable={}",
