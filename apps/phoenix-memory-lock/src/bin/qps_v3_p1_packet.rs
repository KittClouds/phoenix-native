//! V3-P1 sealed positional-authority packet generator.
//!
...
//! System scores, ranks, positional evidence, qrels, and document ids stay in a separate
...
struct HitMeta {
    ...
    rarity: f32,
    positional: f32,
    complete_span_quality: f32,
    ordered_span_quality: f32,
...
fn main() -> Result<()> {
    ...
    let root = PathBuf::from(args.next().context(
        "usage: qps_v3_p1_packet <dataset-root> <packet-json> <ledger-json> [receipt-json] [sampling-salt]",
    )?);
    ...
    let sampling_salt = args.next().unwrap_or_else(|| "v3-p1-v1".to_owned());
    let dataset = root
        .file_name()
    ...
    for (ordinal, (_, candidate_index)) in packet_order.into_iter().enumerate() {
        ...
        let packet_id = format!("p1-{:04}", ordinal + 1);
        ...
        ledger.push(LedgerItem {
            ...
            positional_score: hit.positional,
            complete_span_quality: hit.complete_span_quality,
            ordered_span_quality: hit.ordered_span_quality,
            ordered_fraction: hit.ordered_fraction,
            exact_phrase: hit.exact_phrase,
            exact_field: hit.exact_field,
            matched_group_locality: hit.locality,
        });
    }
    fs::write(&packet_path, serde_json::to_vec_pretty(&packets)?)?;
    fs::write(&ledger_path, serde_json::to_vec_pretty(&ledger)?)?;
    let receipt = Receipt {
        contract: "phoenix.qps.v3-p1-positional-authority-packet/v1",
        dataset,
        ...
        packet_policy:
            "four_blinded_positional_strata_per_selected_query;_only_2_vs_0_becomes_authoritative",
...
fn choose_candidates(
    ...
    index.search_exhaustive_evidence_into(&query.text, EXHAUSTIVE_LIMIT, scratch, &mut hits)?;
    ...
    let candidates = hits
        ...
        .filter_map(|(rank, hit)| {
            ...
            Some(HitMeta {
                ...
                positional: positional_score(hit.rank_evidence_v3.values),
                complete_span_quality: hit.rank_evidence_v3.values[10],
                ordered_span_quality: hit.rank_evidence_v3.values[11],
                ordered_fraction: hit.rank_evidence_v3.values[12],
                exact_phrase: hit.rank_evidence_v3.values[13],
                exact_field: hit.rank_evidence_v3.values[15],
                locality: hit.rank_evidence_v3.values[14],
            })
        })
        .collect::<Vec<_>>();
    ...
    let mut positional_sorted = candidates.clone();
    positional_sorted.sort_unstable_by(|left, right| {
        right
            .positional
            .total_cmp(&left.positional)
            .then_with(|| left.document_index.cmp(&right.document_index))
    });
    let positional_rank = positional_sorted
        .iter()
        .enumerate()
    ...
    if let Some(hit) = disagreement {
        ...
        chosen.push((
            "positional_disagreement".to_owned(),
            if positional_rank[&hit.document_index] < hit.phoenix_rank {
                "positional_high"
            } else {
                "phoenix_high"
            }
    ...
    let high_positional = candidates
        .iter()
        .filter(|hit| !used.contains(&hit.document_index))
        .max_by(|left, right| left.positional.total_cmp(&right.positional))
        .copied();
    if let Some(hit) = high_positional {
        used.insert(hit.document_index);
        chosen.push((
            "high_positional".to_owned(),
            "positional_high".to_owned(),
            hit,
        ));
    }
    if let Some(reference) = high_positional {
        ...
        if let Some(hit) = control {
            used.insert(hit.document_index);
            chosen.push((
                "positional_control".to_owned(),
                "ordinary_positional_matched_rank".to_owned(),
                hit,
            ));
        }
    }
...
#[inline]
fn positional_score(values: [f32; 30]) -> f32 {
    // Frozen packet-selection diagnostic only. Serving remains BM25F-anchored;
    // this private score selects high/ordinary positional evidence strata.
    0.35 * values[10]
        + 0.30 * values[11]
        + 0.15 * values[12]
        + 0.15 * values[13]
...
fn bm25f_rank_for(
    ...
    index.search_exhaustive_evidence_into(
        &query.text,
        limit.min(EXHAUSTIVE_LIMIT),
        scratch,
        &mut hits,
    )?;
