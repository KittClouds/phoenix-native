//! R4-RA Rare Anchor replication microscope.
//!
...
impl Arm {
    const ALL: [Self; 2] = [Self::Anchor, Self::R1Rarest];

    const fn label(self) -> &'static str {
        match self {
            Self::Anchor => "r0_bm25f_anchor",
            Self::R1Rarest => "r1_rarest_matched_term",
            Self::R2RarestCoverage => "r2_rarest_term_times_coverage",
            Self::R3RarityGap => "r3_rarity_distribution_gap",
        }
    }
    ...
    const fn feature_definition(self) -> &'static str {
        match self {
            ...
            Self::R1Rarest => "values[19] rarest_matched_term",
            Self::R2RarestCoverage => {
                "values[19] * values[6] rarest_matched_term times weighted_group_coverage"
            }
            Self::R3RarityGap => "max(values[19] - values[20], 0) rarest minus mean matched rarity",
        }
    }
}
...
fn main() -> Result<()> {
    ...
    let root = PathBuf::from(args.next().context(
        "usage: qps_v3_r3_h_beir <dataset-root> <pairs-json> <authority-json> <gate-json> <output-json>",
    )?);
    ...
    let dataset = root
        ...
        .to_owned();
    if dataset != "fiqa" {
        bail!("R3-H microscope is currently eligible only for fiqa; received {dataset}");
    }
    ...
    let dose_response = ALPHA_GRID
        .iter()
        .copied()
        .map(|alpha| DoseReceipt {
            alpha,
            train_authority: concordance(Arm::R1Rarest, alpha, &train_pairs, &case_index),
            dev_authority: concordance(Arm::R1Rarest, alpha, &dev_pairs, &case_index),
            test_metrics: evaluate(&test_cases, Arm::R1Rarest, alpha).0,
            test_migrations: evaluate(&test_cases, Arm::R1Rarest, alpha).1,
        })
        .collect();

    let receipt = Receipt {
        contract: "phoenix.qps.r4-ra-human-authority-microscope/v1",
        dataset,
        status: "fiqa_r4_ra_replication",
...
fn score(candidate: &Candidate, arm: Arm, alpha: f32) -> f32 {
    let residual = match arm {
        Arm::Anchor => 0.0,
        Arm::R1Rarest => candidate.values[19],
        Arm::R2RarestCoverage => candidate.values[19] * candidate.values[6],
        Arm::R3RarityGap => (candidate.values[19] - candidate.values[20]).max(0.0),
    };
...
fn collect_cases(
    ...
    for (query_id, relevance) in qrels {
        ...
        if limit == TOP_K {
            ...
            index.search_exhaustive_into(&query.text, limit, scratch, &mut hits)?;
        }
        let candidates = hits
            .iter()
            .enumerate()
