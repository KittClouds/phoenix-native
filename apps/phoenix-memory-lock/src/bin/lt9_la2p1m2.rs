...
const REPLAY_SCHEMA: &str = "phoenix.lexical.lt9-la2p1m2-replay/v1";

fn shard(document: u64, documents: u64) -> usize {
    ((document.saturating_mul(lt9_la2p1m2_screen::SHARDS as u64)) / documents)
        .min((lt9_la2p1m2_screen::SHARDS - 1) as u64) as usize
}
...
fn validate_screen(path: &Path) -> Result<(Value, String)> {
    let bytes = std::fs::read(path).with_context(|| format!("read screen {}", path.display()))?;
    let hash = format!("{:x}", Sha256::digest(&bytes));
    let receipt: Value = serde_json::from_slice(&bytes)?;
    ensure!(
        receipt["schema"].as_str() == Some(lt9_la2p1m2_screen::SCREEN_SCHEMA),
        "unexpected P1M2 screen schema"
    );
    ensure!(
        ...
        receipt["frozen_order"] == serde_json::json!(lt9_la2p1m2_screen::SCREEN_ORDER),
        "screen corpus order drift"
    );
    ensure!(
        receipt["discovery_ids"] == serde_json::json!(lt9_la2p1m2_screen::DISCOVERY_IDS),
        "screen discovery cohort drift"
    );
    ensure!(
        receipt["qualification_id"].as_str() == Some(lt9_la2p1m2_screen::QUALIFICATION_ID),
    ...
    for (field, expected) in [
        (
            "minimum_discovery_contested_episodes",
            lt9_la2p1m2_screen::MIN_DISCOVERY_CONTESTED,
        ),
        (
            "minimum_corpora_with_15_contested_episodes",
            lt9_la2p1m2_screen::MIN_CORPORA_WITH_15_CONTESTED,
        ),
        (
            "minimum_candidate_relations_with_contested_episodes",
            lt9_la2p1m2_screen::MIN_DISCOVERY_RELATIONS,
        ),
        (
            "minimum_corpus_shard_cells",
            lt9_la2p1m2_screen::MIN_DISCOVERY_CORPUS_SHARDS,
        ),
    ] {
        ensure!(
    ...
    let rows = receipt["corpora"]
        ...
        rows.len() == lt9_la2p1m2_screen::SCREEN_ORDER.len(),
        "screen corpus count drift"
    );
    for (row, id) in rows.iter().zip(lt9_la2p1m2_screen::SCREEN_ORDER) {
        ensure!(
            row["corpus_id"].as_str() == Some(id),
            "screen row order drift"
        );
        let role = if lt9_la2p1m2_screen::DISCOVERY_IDS.contains(&id) {
            "P1M2_DISCOVERY"
        } else {
            "P1M2Q_RESERVED"
        };
        ensure!(
            row["assigned_role"].as_str() == Some(role),
...
fn replay(args: &[String]) -> Result<()> {
        ...
        args.len() == 2,
        "usage: --replay <screen.json> <output.json>"
    );
    let (screen, screen_hash) = validate_screen(Path::new(&args[0]))?;
    let screen_rows = screen["corpora"]
        .as_array()
        .context("missing screen corpus array")?;
    let mut corpora = Vec::with_capacity(lt9_la2p1m2_screen::DISCOVERY_IDS.len());
    for id in lt9_la2p1m2_screen::DISCOVERY_IDS {
        let row = screen_rows
            .iter()
            .find(|row| row["corpus_id"].as_str() == Some(id))
            .with_context(|| format!("discovery corpus {id} missing from screen"))?;
        ensure!(
            row["assigned_role"].as_str() == Some("P1M2_DISCOVERY"),
            "role drift for {id}"
        );
    ...
    let conclusion = if sufficiency.sufficient {
        "P1M2_DISCOVERY_COHORT_SUFFICIENT; anatomy only; no router selection or qualification"
    } else {
        "P1M2_DISCOVERY_COHORT_OUTCOME_UNDERPOWERED; no router selection; expand only in a new sealed cohort"
    };
    let receipt = ReplayReceipt {
        schema: REPLAY_SCHEMA,
        date: lt9_la2p1m2_screen::DATE,
        scope: "frozen multi-corpus discovery; current hard-abstain distinct-plurality router only; context-only vote is diagnostic; no router or learner update",
        screen_receipt_sha256: screen_hash,
        discovery_ids: lt9_la2p1m2_screen::DISCOVERY_IDS.to_vec(),
        reserved_qualification_id: lt9_la2p1m2_screen::QUALIFICATION_ID,
        outcome_boundary: "expected candidate context is inspected only after the complete seven-corpus discovery cohort and input identities are frozen; no BEIR qrels, queries, relevance labels, ranking, or serving are loaded",
        sufficiency,
        corpora,
        conclusion,
    };
...
fn main() -> Result<()> {
    ...
    match args.get(1).map(String::as_str) {
        Some("--screen") => lt9_la2p1m2_screen::run(&args[2..]),
        Some("--replay") => replay(&args[2..]),
        _ => anyhow::bail!("usage: lt9_la2p1m2 --screen ... | --replay ..."),
    }
}
