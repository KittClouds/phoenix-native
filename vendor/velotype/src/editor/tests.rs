...
struct EmbeddedSaveHost {
    editor: Entity<Editor>,
}

impl EmbeddedSaveHost {
    fn new(requested: std::rc::Rc<std::cell::Cell<bool>>, cx: &mut Context<Self>) -> Self {
        let editor = cx.new(|cx| Editor::embedded_from_markdown(cx, "kernel text".into()));
        cx.subscribe(&editor, move |_, _, event, _| {
            requested.set(*event == EditorEvent::SaveRequested);
        })
        .detach();
        Self { editor }
    }
...
impl Render for EmbeddedSaveHost {
...
async fn embedded_editor_has_no_file_authority_and_preserves_ingress(cx: &mut TestAppContext) {
    ...
    let editor =
        cx.new(|cx| Editor::embedded_from_markdown(cx, "# Phoenix\n\nNative editor.".into()));

    editor.read_with(cx, |editor, cx| {
        assert!(editor.is_embedded());
        assert!(editor.file_path.is_none());
        assert_eq!(editor.markdown_text(cx), "# Phoenix\n\nNative editor.");
    });
}

#[gpui::test]
async fn embedded_save_emits_host_request_without_writing_a_file(cx: &mut TestAppContext) {
    init_editor_test_app(cx);
    let requested = std::rc::Rc::new(std::cell::Cell::new(false));
    let observed = std::rc::Rc::clone(&requested);
    let (host, cx) = cx.add_window_view(move |_window, cx| EmbeddedSaveHost::new(observed, cx));
...
#[gpui::test]
async fn embedded_document_replacement_keeps_host_authority_and_clears_dirty_state(
