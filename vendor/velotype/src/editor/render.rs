...
fn editor_text_font(cx: &App) -> Font {
    ...
    let fallbacks = FALLBACKS
        .get_or_init(|| {
            ...
            FontFallbacks::from_fonts(available_tibetan_font_fallbacks(
                std::env::consts::OS,
                &available,
            ))
        })
        .clone();
    editor_text_font_with_fallbacks(fallbacks)
}

fn editor_text_font_with_fallbacks(fallbacks: FontFallbacks) -> Font {
    let mut font = font(".SystemUIFont");
    font.fallbacks = Some(fallbacks);
    font
}

fn tibetan_font_fallback_candidates(target_os: &str) -> &'static [&'static str] {
    match target_os {
        "windows" => &[
            "Microsoft Himalaya",
            "Noto Serif Tibetan",
...
fn available_tibetan_font_fallbacks(target_os: &str, available: &[String]) -> Vec<String> {
    tibetan_font_fallback_candidates(target_os)
        .iter()
        .filter(|candidate| {
            available
                .iter()
...
impl Render for Editor {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        self.sync_selection_toolbar(window, cx);
        ...
        let editor = cx.entity().downgrade();
        let has_menus = !embedded
            && cx
                .get_menus()
                .map(|menus| !menus.is_empty())
                .unwrap_or(false);
        let titlebar_height = if embedded {
            0.0
        } else {
            custom_titlebar_height(window, d)
        };
        let menu_bar_height = if embedded {
            0.0
        } else {
        ...
        let base = div()
            ...
            .capture_action(cx.listener(Self::on_cross_block_bold))
            .capture_action(cx.listener(Self::on_cross_block_italic))
            .capture_action(cx.listener(Self::on_cross_block_underline))
            .capture_action(cx.listener(Self::on_cross_block_code))
            ...
            .when(!embedded, |base| {
                base.can_drop(|dragged, _window, _cx| dragged.is::<ExternalPaths>())
        ...
        let menus = (!embedded && supports_in_window_menu())
            .then(|| cx.get_menus())
            .flatten()
            .filter(|m| !m.is_empty());
        ...
        let main_content = div()
            ...
            .flex()
            .min_w(px(0.0));
        let main_content = if embedded {
            main_content
        } else {
            if let Some(workspace_panel) =
        ...
        let base = if embedded {
            ...
            if let Some(status_bar) = self.render_status_bar(&theme, &strings, window, cx) {
        ...
        let base = if let Some(toolbar) = self.render_selection_toolbar_overlay(&theme, cx) {
            base.child(toolbar)
        } else {
            base
        };
...
mod tests {
    use super::{
        NoRecentFiles, RenderedRowSpacingInfo, available_tibetan_font_fallbacks,
        callout_row_top_gap, editor_text_font_with_fallbacks, import_menu_split_index,
        ...
        tibetan_font_fallback_candidates,
    };
    use crate::components::{AddLanguageConfig, AddThemeConfig};
    use crate::theme::Theme;
    use gpui::{FontFallbacks, OwnedMenu, OwnedMenuItem};
    use uuid::Uuid;
    ...
    fn editor_text_font_keeps_system_ui_as_primary_family() {
        let font = editor_text_font_with_fallbacks(FontFallbacks::from_fonts(Vec::new()));
    ...
    fn tibetan_font_fallbacks_prioritize_platform_defaults() {
        assert_eq!(
            tibetan_font_fallback_candidates("windows").first().copied(),
            Some("Microsoft Himalaya")
        );
        assert_eq!(
            tibetan_font_fallback_candidates("macos").first().copied(),
            Some("Kailasa")
        );
        assert_eq!(
            tibetan_font_fallback_candidates("linux").first().copied(),
            Some("Noto Serif Tibetan")
        );
        assert_eq!(
            tibetan_font_fallback_candidates("unknown").first().copied(),
            Some("Noto Serif Tibetan")
        );
    }

    #[test]
    fn tibetan_font_fallbacks_exclude_unavailable_families() {
        let available = vec![
            "Segoe UI".to_string(),
            "microsoft himalaya".to_string(),
            "Noto Sans Tibetan".to_string(),
        ];
        assert_eq!(
            available_tibetan_font_fallbacks("windows", &available),
            vec![
                "Microsoft Himalaya".to_string(),
                "Noto Sans Tibetan".to_string()
            ]
