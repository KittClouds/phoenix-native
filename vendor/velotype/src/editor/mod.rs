...
pub struct Editor {
    ...
    selection_toolbar: selection_toolbar::SelectionToolbarState,
    selection_toolbar_requires_selection: bool,
    document_revision: u64,
    semantic_highlight_revision: u64,
    semantic_highlight_mode: SemanticHighlightMode,
    semantic_highlight_blocks: Vec<Entity<Block>>,
    narration_highlight_blocks: Vec<Entity<Block>>,
...
pub struct EntityTagRequest {
    pub kind: EntityTagKind,
    pub custom_kind: Option<SharedString>,
...
impl Editor {
    ...
    pub fn from_markdown(
        ...
        Self::from_markdown_with_host(cx, markdown, file_path, HostMode::Standalone)
    }

    pub fn embedded_from_markdown(cx: &mut Context<Self>, markdown: String) -> Self {
        Self::from_markdown_with_host(cx, markdown, None, HostMode::Embedded)
    }

    pub fn is_embedded(&self) -> bool {
        self.host_mode == HostMode::Embedded
    }

    pub fn markdown_text(&self, cx: &App) -> String {
        self.document.markdown_text(cx)
    }

    pub fn host_document_text(&self, cx: &App) -> String {
    ...
    pub fn mark_embedded_saved(&mut self, cx: &mut Context<Self>) {
        if self.host_mode == HostMode::Embedded {
            self.document_dirty = false;
            cx.notify();
        }
    }

    pub fn replace_embedded_document(&mut self, markdown: String, cx: &mut Context<Self>) {
        if self.host_mode == HostMode::Embedded {
            self.replace_document_from_markdown(markdown, None, cx);
        }
    }

    fn from_markdown_with_host(
        cx: &mut Context<Self>,
        markdown: String,
        file_path: Option<PathBuf>,
        host_mode: HostMode,
        ...
        let mut editor = Self {
            ...
            selection_toolbar: selection_toolbar::SelectionToolbarState::default(),
            selection_toolbar_requires_selection: false,
            document_revision: 0,
            semantic_highlight_revision: 0,
            semantic_highlight_mode: SemanticHighlightMode::Subtle,
            semantic_highlight_blocks: Vec::new(),
            narration_highlight_blocks: Vec::new(),
            menu_bar_open: None,
