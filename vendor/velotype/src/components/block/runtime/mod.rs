...
use super::{
    BlockEvent, BlockKind, BlockRecord, BlockSemanticHighlights, CalloutVariant, FootnoteRegistry,
    InlineFootnoteHit, SemanticHighlight, SemanticHighlightMode, UndoCaptureKind,
};
...
use crate::components::markdown::inline::{
    ...
    InlineStyle, InlineTextTree, SelectionLinkState, SelectionMarkState, StyleFlag,
};
...
#[inline]
fn caret_opacity_at_elapsed(elapsed: Duration) -> f32 {
    let phase = elapsed.as_millis() / CARET_BLINK_HALF_PERIOD.as_millis();
...
pub struct Block {
    ...
    pub(crate) semantic_highlights: BlockSemanticHighlights,
    pub(crate) narration_spans: Vec<SemanticHighlight>,
...
impl Block {
    pub fn with_record(cx: &mut Context<Self>, record: BlockRecord) -> Self {
        ...
        let mut block = Self {
            ...
            render_cache,
            code_highlight: None,
            semantic_highlights: BlockSemanticHighlights::default(),
            narration_spans: Vec::new(),
            children: Vec::new(),
    ...
    pub(crate) fn semantic_highlights(&self) -> &BlockSemanticHighlights {
    ...
    pub(crate) fn set_semantic_highlights(
        ...
        self.semantic_highlights = BlockSemanticHighlights {
            revision,
            mode,
            spans,
        };
    ...
    pub(crate) fn set_semantic_highlight_mode(&mut self, mode: SemanticHighlightMode) -> bool {
        if self.semantic_highlights.mode == mode {
            return false;
        }
        self.semantic_highlights.mode = mode;
        self.semantic_highlights.revision = self.semantic_highlights.revision.wrapping_add(1);
        true
    }

    pub(crate) fn clear_semantic_highlights(&mut self) -> bool {
    ...
    pub(crate) fn selection_style_state(
        &self,
        ...
        self.record
            .title
            .style_state(self.current_to_clean_range(current_range), flag)
    }

    pub(crate) fn set_selection_style(
        &mut self,
        current_range: Range<usize>,
        flag: StyleFlag,
        enabled: bool,
        ...
        let clean_range = self.current_to_clean_range(current_range);
        let mut next_title = self.record.title.clone();
        if !next_title.set_style_enabled(clean_range, flag, enabled) {
            return false;
        }
        self.record.set_title(next_title);
        self.sync_render_cache();
    ...
    pub(crate) fn selection_link_state(&self, current_range: Range<usize>) -> SelectionLinkState {
        if self.uses_raw_text_editing() {
            return SelectionLinkState::Unavailable;
        }
        self.record
            .title
            .inline_link_state(self.current_to_clean_range(current_range))
    }

    pub(crate) fn set_selection_link(
        &mut self,
        current_range: Range<usize>,
        ...
        let clean_range = self.current_to_clean_range(current_range);
        let mut next_title = self.record.title.clone();
        if !next_title.set_inline_link(clean_range, destination) {
            return false;
        }
        self.record.set_title(next_title);
        self.sync_render_cache();
    ...
    pub fn move_to_with_preferred_x(
        ...
        let previous_reversed = self.selection_reversed;
        self.assign_collapsed_selection_offset(
            offset,
            CollapsedCaretAffinity::Default,
            preferred_x,
        ...
        self.cursor_blink_epoch = Instant::now();
        if self.selected_range != previous_range || self.selection_reversed != previous_reversed {
            cx.emit(BlockEvent::SelectionChanged);
        }
        cx.notify();
    }

    /// Starts a transition-driven cursor blink loop.
    /// The caret has only two observable states, so repaint only at each
    /// 500 ms state boundary. A smooth 33 ms loop makes an entire long-note
    /// projection/render pass part of an otherwise cosmetic animation.
    ///
    ...
    /// Binary caret blink. Opacity remains constant between transitions, so
    /// intermediate editor renders are unnecessary.
    pub fn cursor_opacity(&self) -> f32 {
        caret_opacity_at_elapsed(self.cursor_blink_epoch.elapsed())
    }
    ...
    pub(crate) fn selection_bounds_for_range(
        &self,
        current_range: Range<usize>,
    ...
    pub(crate) fn pointer_selection_active(&self) -> bool {
        self.is_selecting || self.code_language_is_selecting
    }
}
