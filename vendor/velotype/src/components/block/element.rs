...
use super::{
    Block, InlineFootnoteHit, InlineLinkHit, SemanticHighlight, SemanticHighlightMode,
...
const SUBTLE_GRADIENT_STEPS: usize = 6;
...
fn build_text_runs(
    ...
    let semantic = input.semantic_highlights();
    ...
    if semantic.mode != SemanticHighlightMode::Off {
        for span in &semantic.spans {
            push_semantic_boundaries(display_text, span, &mut boundaries);
        }
    }
    ...
    for boundary_pair in boundaries.windows(2) {
        ...
        while semantic_idx < semantic.spans.len() && semantic.spans[semantic_idx].range.end <= start
        {
            semantic_idx += 1;
        }
        let active_semantic = semantic
            .spans
            .get(semantic_idx)
            .filter(|span| span.range.start <= start && start < span.range.end);
        ...
        if let Some(span) = active_semantic {
...
fn push_semantic_boundaries(text: &str, span: &SemanticHighlight, boundaries: &mut Vec<usize>) {
    boundaries.push(span.range.start);
    let slice = &text[span.range.clone()];
    let grapheme_count = slice.graphemes(true).count();
    if grapheme_count <= 1 {
        boundaries.push(span.range.end);
        return;
    }
    let stride = grapheme_count.div_ceil(SUBTLE_GRADIENT_STEPS);
    for (index, (offset, _)) in slice.grapheme_indices(true).enumerate() {
        if index > 0 && index % stride == 0 {
            boundaries.push(span.range.start + offset);
        }
    }
    boundaries.push(span.range.end);
}

fn semantic_gradient_color(span: &SemanticHighlight, offset: usize) -> Hsla {
    let length = span.range.end.saturating_sub(span.range.start).max(1);
    let progress = (offset.saturating_sub(span.range.start) as f32 / length as f32).clamp(0.0, 1.0);
    let mut color = [0.0; 4];
...
pub struct PrepaintState {
    ...
    semantic_backgrounds: Vec<PaintQuad>,
    line_height: Pixels,
    hitbox: Hitbox,
}
...
impl Element for BlockTextElement {
    ...
    fn prepaint(
        ...
        let mut semantic_backgrounds = Vec::new();
        if !self.is_placeholder
            && !input.is_source_raw_mode()
            && input.semantic_highlights().mode == SemanticHighlightMode::Vivid
        {
            let text = input.display_text();
            let pad_x = px(3.0);
            let pad_y = px(1.0);
            ...
            for span in &input.semantic_highlights().spans {
                ...
                for segment in range_segment_bounds(
                    ...
                    let outer = Bounds::from_corners(
                        point(
                            segment.left() - pad_x - px(1.0),
                            segment.top() - pad_y - px(1.0),
                        ),
                        point(
                            segment.right() + pad_x + px(1.0),
                            segment.bottom() + pad_y + px(1.0),
                        ),
                    );
                    ...
                    let mut border = fill(outer, border_color);
                    border.corner_radii = Corners::all(radius + px(1.0));
                    semantic_backgrounds.push(border);
                    let mut background = fill(inner, background_color);
                    background.corner_radii = Corners::all(radius);
                    semantic_backgrounds.push(background);
                }
            }
        }
    ...
    fn paint(
        ...
        for semantic_bg in prepaint.semantic_backgrounds.drain(..) {
            window.paint_quad(semantic_bg);
        }
...
mod tests {
    ...
    #[test]
    fn subtle_gradient_boundaries_are_bounded_and_utf8_aligned() {
        let text = "before 中文😀かな after";
        let start = text.find("中文").expect("fixture");
        let end = text.find(" after").expect("fixture");
        let span = SemanticHighlight::new(start..end, [0.1, 0.8, 0.4, 1.0], [0.2, 0.5, 1.0, 1.0]);
        let mut boundaries = Vec::new();
        push_semantic_boundaries(text, &span, &mut boundaries);
        assert!(boundaries.len() <= super::SUBTLE_GRADIENT_STEPS + 2);
