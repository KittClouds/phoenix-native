...
impl InlineTextTree {
    ...
    pub(crate) fn style_state(&self, range: Range<usize>, flag: StyleFlag) -> SelectionMarkState {
        ...
        for fragment in &self.fragments {
            let fragment_end = consumed + fragment.text.len();
            if consumed < clamped_end && fragment_end > clamped_start && !fragment.text.is_empty() {
                if style_flag_enabled(fragment.style, flag) {
                    any_enabled = true;
                } else {
                    any_disabled = true;
                }
        ...
        match (any_enabled, any_disabled) {
            (true, false) => SelectionMarkState::On,
            (false, true) => SelectionMarkState::Off,
            (true, true) => SelectionMarkState::Mixed,
            (false, false) => SelectionMarkState::Unavailable,
        }
    }

    pub(crate) fn set_style_enabled(
        &mut self,
        range: Range<usize>,
        flag: StyleFlag,
        enabled: bool,
        ...
        for fragment in &mut middle.fragments {
            let next = set_style_flag(fragment.style, flag, enabled);
            changed |= next != fragment.style;
            fragment.style = next;
        }
        if !changed {
            return false;
    ...
    pub(crate) fn inline_link_state(&self, range: Range<usize>) -> SelectionLinkState {
        let clamped_start = range.start.min(self.visible_len());
        let clamped_end = range.end.min(self.visible_len());
        if clamped_start >= clamped_end {
        ...
        for fragment in &self.fragments {
            ...
            if consumed < clamped_end && fragment_end > clamped_start && !fragment.text.is_empty() {
                saw_fragment = true;
                let Some(link) = fragment.link.as_ref() else {
                    return SelectionLinkState::Off;
                };
                let next = link.open_target();
                if destination.is_some_and(|current| current != next) {
                    return SelectionLinkState::Mixed;
                }
        ...
        match (saw_fragment, destination) {
            (true, Some(destination)) => SelectionLinkState::On(destination.to_owned()),
            _ => SelectionLinkState::Unavailable,
        }
    }
    ...
    pub(crate) fn set_inline_link(
        ...
        if middle
            .fragments
            .iter()
            .any(|fragment| fragment.math.is_some() || fragment.footnote.is_some())
        {
            return false;
        }
        ...
        let mut changed = false;
        for fragment in &mut middle.fragments {
            changed |= fragment.link != next_link;
...
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub(crate) enum SelectionMarkState {
    Off,
    On,
    Mixed,
    #[default]
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum SelectionLinkState {
    Off,
    On(String),
    Mixed,
    Unavailable,
