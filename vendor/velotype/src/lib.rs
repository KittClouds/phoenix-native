...
/// Install the editor globals and key bindings without filesystem preferences,
/// network clients, application menus, or standalone window authority.
pub fn init_embedded(cx: &mut App) {
    let preferences = config::preferences::AppPreferences::default();
    ...
    config::EditorSettings::init_embedded(cx, preferences.show_table_headers);
