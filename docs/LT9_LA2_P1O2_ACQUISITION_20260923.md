...
This is not yet a human-reviewed compatibility result or an observability verdict. The 30-row set can test only `save ↔ spare`; it cannot support a claim across all five conditional candidates. After labels are saved, validate packet IDs and labels first. Apply the frozen per-candidate fit/holdout sufficiency gate before any feature join; a candidate missing either graph's label floor stays underpowered. No authority or retrieval work is authorized.
